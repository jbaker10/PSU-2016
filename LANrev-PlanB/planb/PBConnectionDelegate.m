...
NSString *const kConnectionDelegateHost = @"hostname.domain.com";
...
- (NSURLCredential *)serverCredentialForProtectionSpace:(NSURLProtectionSpace *)protectionSpace {
  ...
  if (firstCert) {
    ...
    PBLog(@"Server certificate details: CN: domain.com, Org: %@, OU: %@, ValidFrom: %@, ValidUntil: %@, "
          @"SHA1: %@, Issuer CN: %@",
          cert.orgName, cert.orgUnit, cert.validFrom,
